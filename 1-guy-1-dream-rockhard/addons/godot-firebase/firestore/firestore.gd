...
func _pooled_request(task: FirestoreTask) -> void:
	...
	if not Firebase.emulating:
		if auth == null or not auth.has("idtoken"):
			Firebase._printerr("Firestore request aborted: no auth token available")
			return
...
func _handle_task_finished(task: FirestoreTask, timeout_seconds := 2.0):
	# Wait for the task to finish but don't block forever — add a timeout.
	...
	if not task.finished:
		var err = {"error": {"code": 0, "status": "Timeout", "message": "Request timed out after %s seconds" % [str(timeout_seconds)]}}
		task.error = err
		error.emit(task.error)
		# Return empty array instead of nil for Array return types
		if task.action == FirestoreTask.Task.TASK_QUERY or task.action == FirestoreTask.Task.TASK_LIST:
			return []
		return null

	if task.error != null and task.error.keys().size() > 0:
		error.emit(task.error)
		# Return empty array instead of nil for Array return types
		if task.action == FirestoreTask.Task.TASK_QUERY or task.action == FirestoreTask.Task.TASK_LIST:
			return []
