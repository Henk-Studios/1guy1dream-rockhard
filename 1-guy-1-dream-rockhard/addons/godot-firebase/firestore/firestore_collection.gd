...
func _process_request(task : FirestoreTask, document_id : String, url : String, fields := "") -> void:
	if auth == null or auth.is_empty():
		...
		var auth_result_code = result[0]
		if typeof(auth_result_code) != TYPE_INT:
			auth_result_code = ERR_CANT_CONNECT
		if auth_result_code != 1:
			Firebase.Firestore._check_auth_error(auth_result_code, result[1])
			return
