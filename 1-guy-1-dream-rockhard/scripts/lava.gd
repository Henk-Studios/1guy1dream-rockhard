...
func _check_player_touch() -> void:
	...
	if World.the_guy.global_position.y >= lava_surface_y:
		...
		if rising_lava and not _lava_touch_triggered:
			...
			var current_record = Manager.utility.get_personal_record(Manager.utility.GAME_TYPE_INFINITE_RISING_LAVA, seed_type)
			if World.main.time_elapsed > current_record:
				Manager.utility.set_personal_record(World.main.time_elapsed, Manager.utility.GAME_TYPE_INFINITE_RISING_LAVA, seed_type)
